...
//CODE HERE
bestMovie = i => i + " is the best movie ever!";
...
function matcher(){
    for (let i = 0; i < classes.length; i++){
      for (let j = i+1; j < classes[i].length; j++){
        classes[i][j] = (classes[i][j])
      }
...
//CODE HERE
function Dog(name, age, bread, tricks)  {
    this.name = name
    this.age = age
    this.breed = breed
    this.tricks = tricks
...
//CODE HERE
function Phone(brand, model, color, sold)  {
    this.brand = brand
    this.model = model
...
//CODE HERE
  let phone1 = new Phone("Sprint", "A", 128, "blue", false)
  
  let phone2 = new Phone("Verizon", "B", 256, "red", false)
  
  let phone3 = new Phone("Tmobile", "C", 512, "yellow", false)
